...
export default function YourShop() {
    ...
    const deleteOneMutation = useMutation({
        ...
        mutationFn: async (_id: string) => await deleteData(`${import.meta.env.VITE_API_BASE_URL}/product/delete/${_id}`),
        onSuccess: () => {
    ...
    const deleteAllMutation = useMutation({
        onMutate: () => setIsDeleting(true),
        mutationFn: async () => await deleteData(`${import.meta.env.VITE_API_BASE_URL}/product/deletes/${user_id}`),
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['all-products'] });
            queryClient.invalidateQueries({ queryKey: [`your-products-${user_id}`] });
        },
